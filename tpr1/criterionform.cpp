...
void CriterionForm::calculateVectors()
{
    ...
    for(int i=0; i < kol; i++)
    {
        for(int j=0; j < kol; j++)
        {
            ownVectors[i] = ownVectors[i] * tableContent[i][j];
        }
        ownVectors[i] = pow(ownVectors[i], 1.0/kol);
    }
    //дальше нормализация
    float sum=0;
    for(int i=0;i<kol;i++)
    ...
    for(int i=0;i<kol;i++)//*********
    {
        ownVectors[i] = ownVectors[i]/sum;
    }
    //индекс согласованости
    QVector< float >colunSum;//сумма по столбцам
    for(int i=0;i<kol;i++)
    {
        colunSum.append(0);
    }
    for(int i=0;i<kol;i++)
    {
        for(int j=0;j<kol;j++)
        {
            colunSum[i] = colunSum[i] + tableContent[j][i];
        }
    }
    float maxL=0;
    for(int i=0;i<kol;i++)//
    {
        colunSum[i] = colunSum[i]*ownVectors[i];
        maxL+=colunSum[i];
    }
    //
    indeksS=(maxL-kol);
    if(indeksS > 0.0001)
    {
        indeksS=indeksS/(kol-1)/matrixS[kol-1]*100;//kol-1
    }
    else
        indeksS=0;
